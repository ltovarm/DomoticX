...
class TestConsumer(unittest.TestCase):
    ...
    def test_consumer_send_to_database(self):
            ...
            for i, row in enumerate(reversed(rows)):
                ...
                self.assertEqual(self.message['datajson'][i]['id'], received_message['id'], "The content of the message is not as expected.")
                self.assertEqual(self.message['datajson'][i]['msgtype'], received_message['msgtype'], "The content of the message is not as expected.")
                self.assertEqual(self.message['datajson'][i]['data'], received_message['data'], "The content of the message is not as expected.")
    ...
    async def receive_from_websocket(self):
        async with websockets.connect('ws://localhost:8080/ws') as websocket:
            while True:
                ...
                self.assertEqual(self.message['id'], received_message['id'], "The content of the message is not as expected.")
                self.assertEqual(self.message['ndata'], received_message['ndata'], "The content of the message is not as expected.")
                for i in range(len(self.message['datajson'])):
                    self.assertEqual(self.message['datajson'][i]['id'], received_message['datajson'][i]['id'], "The content of the message is not as expected.")
                    self.assertEqual(self.message['datajson'][i]['msgtype'], received_message['datajson'][i]['msgtype'], "The content of the message is not as expected.")
                    self.assertEqual(self.message['datajson'][i]['data'], received_message['datajson'][i]['data'], "The content of the message is not as expected.")
